...
function findClosestMinecraftBlock(glowBlock_list,r, g, b) {
    let closestBlock = { name: "air.png", color: [0,0,0], number:999};
...
document.getElementById('submitButton').addEventListener('click', function() {
    ...
    if (dotCanvas.width > dotCanvas.height) {
        // 横長
        const canvasWidth = 70; // vw
        const canvasHeight = 70 * aspect_rate; // vw
      
        dotCanvas.style.width = `${canvasWidth}vw`;
        dotCanvas.style.height = `${canvasHeight}vw`;
      
        popup_content[0].style.width = `${canvasWidth}vw`;
        popup_content[0].style.height = `calc(${canvasHeight}vw + 50px)`;
      
      } else {
        // 縦長
        const canvasHeight = 70; // vh
        const canvasWidth = 70 / aspect_rate; // vh
      
        dotCanvas.style.width = `${canvasWidth}vh`;
        dotCanvas.style.height = `${canvasHeight}vh`;
      
        popup_content[0].style.width = `${canvasWidth}vh`;
        popup_content[0].style.height = `calc(${canvasHeight}vh + 50px)`;
      }
